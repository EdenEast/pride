...
inline bool is_terminal(const std::streambuf* os_buffer)
{
    ...
    if (os_buffer == std::cout.rdbuf())
    {
        static const bool cout_term = (_isatty(_fileno(stdout)) || internal::is_msys_pty(_fileno(stdout)));
        return cout_term;
    }
    else if (os_buffer == std::cerr.rdbuf())
    {
        static const bool cerr_term = (_isatty(_fileno(stderr)) || internal::is_msys_pty(_fileno(stderr)));
        return cerr_term;
    }
#endif
    return false;
...
namespace internal
{
    ...
    inline bool support_ansi(const std::streambuf* stream) noexcept
    {
        if (stream == std::cout.rdbuf())
        {
            static const bool cout_ansi = (is_msys_pty(_fileno(stdout))) || set_win_term_ansi_colors(stream);
            return cout_ansi;
        }
        else if (stream == std::cerr.rdbuf() || stream == std::clog.rdbuf())
        {
            static const bool cerr_ansi = (is_msys_pty(_fileno(stderr))) || set_win_term_ansi_colors(stream);
            return cerr_ansi;
        }
        return false;
    }
