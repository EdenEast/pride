...
struct violation_info_t
{
    ...
    violation_info_t(const char* file, int line, const char* function,
        const char* expr_str, internal::bool_expression_t expr) noexcept;
...
inline violation_info_t::violation_info_t(
    ...
    const char* function,
    const char* expr_str,
    internal::bool_expression_t expr) noexcept
    : line_number(line)
    , file_name(file)
