...
{
    inline std::ostream& assert_fail_default_log(std::ostream& os, const impl::violation_info_t& info) noexcept
    {
        ...
        os  << std::boolalpha << style_t::bold
            ...
            << fg_t::reset    << style_t::reset << " `"
                              << impl::expression_output_t(info.commit, info.expression)
    ...
    inline void assert_fail_default_log(const impl::violation_info_t& info) noexcept
    {
        // Must flush because we will abort()
        assert_fail_default_log(std::cerr, info).flush();
    }

    inline std::string assert_fail_to_string(const impl::violation_info_t& info)
    {
    ...
    {
    public:
        assert_message_t(impl::violation_info_t& info);
        friend std::ostream& operator<<(std::ostream& os, assert_message_t& msg);
        impl::violation_info_t info;
    };

    inline assert_message_t::assert_message_t(impl::violation_info_t& info)
    ...
    inline void assert_failed_default(impl::violation_info_t info) noexcept
    {
        using namespace std;

        {
            ...
            impl::violation_info_t _info(std::move(info));
    ...
    {
        assert_failed_default(impl::violation_info_t(file, line, function, expr_str, std::move(expr)));
    }
}}
