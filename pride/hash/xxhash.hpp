...
namespace internal::xxhash
{
    ...
    {
        return pride::detection::is_little_endian() ? *(uint32_t*)ptr : swap32(*(uint32_t*)ptr);
    }

    inline uint64_t read64(const void* ptr)
    {
        return pride::detection::is_little_endian() ? *(uint64_t*)ptr : swap64(*(uint64_t*)ptr);
    }
    ...
    {
        static constexpr std::array<uint32_t, 5> primes{ { 2654435761U, 2246822519U, 3266489917U, 668265263U, 374761393U } };
        ...
        if (len >= 16)
        {
            ...
            uint32_t v1 = seed + primes[0] + primes[1];
            uint32_t v2 = seed + primes[1];
            uint32_t v3 = seed + 0;
            uint32_t v4 = seed - primes[0];

            do
            {
                v1 += read32(p) * primes[1];
                v1 = rotl32(v1, 13);
                v1 *= primes[0];
                p += 4;
                v2 += read32(p) * primes[1];
                v2 = rotl32(v2, 13);
                v2 *= primes[0];
                p += 4;
                v3 += read32(p) * primes[1];
                v3 = rotl32(v3, 13);
                v3 *= primes[0];
                p += 4;
                v4 += read32(p) * primes[1];
                v4 = rotl32(v4, 13);
                v4 *= primes[0];
                p += 4;
            } while (p <= limit);
        ...
        while (p + 4 <= end)
        {
            h32 += read32(p) * primes[2];
            h32 = rotl32(h32, 17) * primes[3];
            p += 4;
        }
    ...
    {
        static constexpr std::array<uint64_t, 5> primes{ { 11400714785074694791ULL, 14029467366897019727ULL, 1609587929392839161ULL, 9650029242287828579ULL, 2870177450012600261ULL } };
        const uint8_t* p = (const uint8_t*)input;
        const uint8_t* end = p + len;
        uint64_t h64;
        ...
        if (len >= 32)
        {
            ...
            uint64_t v1 = seed + primes[0] + primes[1];
            uint64_t v2 = seed + primes[1];
            uint64_t v3 = seed + 0;
            uint64_t v4 = seed - primes[0];

            do
            {
                v1 += read64(p) * primes[1];
                p += 8;
                v1 = rotl64(v1, 31);
                v1 *= primes[0];
                v2 += read64(p) * primes[1];
                p += 8;
                v2 = rotl64(v2, 31);
                v2 *= primes[0];
                v3 += read64(p) * primes[1];
                p += 8;
                v3 = rotl64(v3, 31);
                v3 *= primes[0];
                v4 += read64(p) * primes[1];
                p += 8;
                v4 = rotl64(v4, 31);
                v4 *= primes[0];
            } while (p <= limit);
        ...
        while (p + 8 <= end)
        {
            ...
            k1 *= primes[0];
            h64 ^= k1;
            h64 = rotl64(h64, 27) * primes[0] + primes[3];
            p += 8;
        }

        if (p + 4 <= end)
        {
            h64 ^= (uint64_t)(read32(p)) * primes[0];
            h64 = rotl64(h64, 23) * primes[1] + primes[2];
            p += 4;
        }
