...
namespace pride::hash
{
    namespace detail::mm3
    {
        template<typename T>
        struct constant {};
        template<> struct constant<uint32_t> { static constexpr uint32_t seed = 0x6384BA69; };
        template<> struct constant<uint64_t> { static constexpr uint64_t seed = 0x7389ab371dea3274ULL; };
        ...
        template<>
        uint32_t compute<uint32_t>(const void* key, size_t len, uint32_t seed)
        {
        ...
        template<>
        uint64_t compute<uint64_t>(const void* key, size_t len, uint64_t seed)
        {
