...
namespace pride { namespace ct
{
    ...
    // --------------------------------------------------------------------------------------------
    // Applying function to a sequence
    ...
    struct functor
    {
        ...
        template<template<typename...> class S, typename... Us>
        struct apply<S<Us...>>
        {
            using type = Seq<Us...>;
        };
    ...
    namespace internal
    {
        ...
        {
            [](...){}(std::array<int, sizeof...(Ts) + 1>{(function(pride::ct::identity<Ts>(), pride::ct::size_t<Indices>()), 0)..., 0});
        }
    }
